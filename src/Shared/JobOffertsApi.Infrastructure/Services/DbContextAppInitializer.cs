...
namespace JobOffersApi.Infrastructure.Services;
...
public class DbContextAppInitializer : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        var dbContextTypes = AppDomain.CurrentDomain.GetAssemblies()
            ...
            .Where(x => typeof(BaseDbContext).IsAssignableFrom(x) && !x.IsInterface && x != typeof(BaseDbContext));

        using var scope = _serviceProvider.CreateScope();
        foreach (var dbContextType in dbContextTypes)
        {
            var dbContext = scope.ServiceProvider.GetService(dbContextType) as BaseDbContext;
